...
fastify.get("/", async () => { 
    return { status: "MIRAR API online" };
});
...
fastify.post("/mirar/change/:fromBot/:toBot/:userId/:amount", async (req, reply) => {
    ...
    if (fromBotData.status !== "approved" || toBotData.status !== "approved") {
        return reply.code(403).send({ error: "Both bots must be approved" });
    }

    const fromWallet = `mirar:wallet:${fromBot}:${userId}`;
    const toWallet = `mirar:wallet:${toBot}:${userId}`;

    const fromBalance = Number(await redis.hget(fromWallet, "balance")) || 0;
    ...
    await redis.hset(fromWallet, { balance: fromBalance - amt });
    const toBalance = Number(await redis.hget(toWallet, "balance")) || 0;
    await redis.hset(toWallet, { balance: toBalance + converted });
...
// NOVA ROTA: consultar saldo
fastify.get("/mirar/wallet/:botId/:userId", async (req, reply) => {
    ...
    const balance = Number(await redis.hget(walletKey, "balance")) || 0;
...
fastify.register(async (team) => {
    team.addHook("preHandler", async (req, reply) => {
        if (req.headers.authorization !== `Bearer ${process.env.TEAM_TOKEN}`) {
            return reply.code(403).send({ error: "Restricted to MIRAR Team" });
        }
    });
